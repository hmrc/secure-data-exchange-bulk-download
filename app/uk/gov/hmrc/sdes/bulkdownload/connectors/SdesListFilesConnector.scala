...
@Singleton
class SdesListFilesConnector @Inject() (servicesConfig: ServicesConfig, http: HttpClientV2)(implicit ec: ExecutionContext) extends Logging {
  ...
  lazy val serviceUrl: String = {
    ...
    val context    = servicesConfig.getConfString(contextKey, throw new IllegalStateException(s"Service context missing: $contextKey"))
    s"$baseUrl$context"
  }
  ...
  def listAvailableFiles(fileType: String)(implicit hc: HeaderCarrier): Future[List[FileItem]] = {
    ...
    http.get(url"$url").execute[List[FileItem]]
  }
